...
for (const { entry, module: moduleName } of entries) {
	const tmpFile = join(
		tmpdir(),
		`react-tela-${moduleName.replace(/\//g, '-')}.d.ts`,
	);
	...
	execFileSync(
		...
		{
			cwd: resolve(packagesDir, 'react-tela'),
...
const reactIndexDts = readFileSync(
	resolve(reactTypesDir, 'index.d.ts'),
	'utf-8',
);
const reactGlobalDts = readFileSync(
	resolve(reactTypesDir, 'global.d.ts'),
	'utf-8',
);
